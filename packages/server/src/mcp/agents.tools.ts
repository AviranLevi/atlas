...
export function registerAgentTools(server: McpServer) {
  server.registerTool('list_agents', {
    description: 'List all available AI agents with their id, name, and description. Optionally filter by projectId to return only agents assigned to that project.',
    inputSchema: z.object({
      projectId: z.string().uuid().optional().describe('Optional project UUID to filter agents by project assignment'),
    }),
  }, async ({ projectId }) => {
    const agents = projectId
      ? await agentsService.listByProject(projectId)
  ...
  server.registerTool('get_agent_context', {
    ...
    inputSchema: z.object({
      ...
      projectId: z.string().uuid().optional().describe('Optional project UUID to include project-scoped skills and rules'),
    }),
