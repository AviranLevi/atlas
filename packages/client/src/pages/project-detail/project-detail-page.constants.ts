...
export const taskStatusConfig: Record<string, { icon: React.ReactNode; className: string }> = {
  ...
  'In Progress': { icon: React.createElement(Loader2, { className: 'h-4 w-4' }), className: 'text-blue-600 dark:text-blue-400' },
  'In Review': { icon: React.createElement(Clock, { className: 'h-4 w-4' }), className: 'text-yellow-600 dark:text-yellow-400' },
  'Done': { icon: React.createElement(CheckCircle2, { className: 'h-4 w-4' }), className: 'text-green-600 dark:text-green-400' },
};
