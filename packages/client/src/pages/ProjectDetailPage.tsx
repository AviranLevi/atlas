...
// Types
import type { ProjectStatus, Task } from '@my-agents/shared';
...
const taskStatusConfig: Record<string, { icon: React.ReactNode; className: string }> = {
  'To Do': { icon: <ClipboardList className="h-4 w-4" />, className: 'text-muted-foreground' },
  'In Progress': { icon: <Loader2 className="h-4 w-4" />, className: 'text-blue-600 dark:text-blue-400' },
  'In Review': { icon: <Clock className="h-4 w-4" />, className: 'text-yellow-600 dark:text-yellow-400' },
  'Done': { icon: <CheckCircle2 className="h-4 w-4" />, className: 'text-green-600 dark:text-green-400' },
};
