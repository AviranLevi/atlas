    ...
    <div className="rounded-lg border border-border/50 bg-muted/20 text-xs animate-fade-in-up overflow-hidden">
      ...
      {open && (
        <div className="border-t border-border/40 px-3 py-2 space-y-1.5">
          {steps.length === 0 && (
            <p className="text-muted-foreground/50 py-0.5">Waiting…</p>
          )}
          {steps.map((step) => (
            <div
              key={step.id}
              className="flex items-baseline gap-1.5 font-mono animate-fade-in-up"
            >
              <span className="shrink-0 text-muted-foreground/40">▸</span>
              <span className="font-semibold text-muted-foreground">{step.toolName}</span>
              {step.hint && (
                <span className="truncate text-muted-foreground/50">{step.hint}</span>
              )}
              {step.status === 'pending' && (
