    ...
    <div className={cn('prose prose-sm prose-invert max-w-none', className)}>
      <ReactMarkdown
        components={{
          ...
          ul: ({ children }) => <ul className="text-sm text-muted-foreground list-disc pl-4 mb-2 space-y-0.5">{children}</ul>,
          ol: ({ children }) => <ol className="text-sm text-muted-foreground list-decimal pl-4 mb-2 space-y-0.5">{children}</ol>,
